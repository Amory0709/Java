...
public class Sort {
	...
	public static int findIndexOfLexicographicallyEarliestValue(String[] array, int fromIndex) {
		String smallest = array[fromIndex];
		int indexOfSmallest = fromIndex;
		for (int i = fromIndex+1; i < array.length;++i) {
			if (Strings.isLessThan(array[i], smallest)) {
				smallest = array[i];
				indexOfSmallest = i;
			}
		}//end of loop
	...
	public static void selectionSortInPlace(String[] array) {
		for(int j = 0;j<array.length-1;++j) {
			int index = findIndexOfLexicographicallyEarliestValue(array,j);
			//swap array[j] and array[index]
			swapValuesAtIndicesInPlace(array,j,index);

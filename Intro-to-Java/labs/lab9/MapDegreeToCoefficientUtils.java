...
public class MapDegreeToCoefficientUtils {
	...
	public static Map<Integer, Double> createNextMap(double coefficient, int degree, Map<Integer, Double> prevMap) {
		Map<Integer,Double> editMap = copyMap(prevMap);
		if(editMap.get(degree) != null) {
			editMap.replace(degree, editMap.get(degree)+coefficient);
		}else {
			editMap.put(degree, coefficient);
		}
		return editMap;
	}
}

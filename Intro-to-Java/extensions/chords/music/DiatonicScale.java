...
public class DiatonicScale {
	...
	public Pitch getPitch(int offset) {
		if(offset >= 0) {
			int [] dictonicIndex = {0,2,4,5,7,9,11};
			int step = dictonicIndex[offset % 7];
			int round = offset/7;
			return p.getChromaticRelative(step + round * 12); 
		}else {
			int []dictonicIndex = {-1,-3,-5,-7,-8,-10,-12};
			int round = offset/8;
			int step =  dictonicIndex[(Math.abs(offset)-1)%7];
			return p.getChromaticRelative(step - round*12);
		}
